...
> [...] to estimate causal effects, we must find or create a situation in which the treatment and control groups are comparable with respect to all the variables that might affectt the outcome other than the treatment variable itself. Only when this assumption is satisfied can we use the average factual or observed outcome of one group as a good estimate of the average counterfactual outcome of the other group. (p. 130)
